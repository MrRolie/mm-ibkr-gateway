...
class SymbolSpec(BaseModel):
    ...
    exchange: Optional[str] = Field(None, description="Preferred exchange or routing, e.g. 'SMART', 'GLOBEX'.")
    currency: Optional[str] = Field(None, description="Currency code, e.g. 'USD'.")
    expiry: Optional[str] = Field(None, description="Contract expiry in YYYY-MM-DD format for derivatives.")
    ...
    multiplier: Optional[str] = Field(None, description="Contract multiplier as string (IBKR-style).")
...
class Bar(BaseModel):
    ...
    barSize: str = Field(..., description="Bar size as human string, e.g. '1 min', '5 mins', '1 day'.")
...
class AccountSummary(BaseModel):
    ...
    maintenanceMargin: float = Field(0.0, ge=0, description="Current maintenance margin requirement.")
...
class Position(BaseModel):
    ...
    quantity: float = Field(..., description="Position size (positive for long, negative for short).")
    avgPrice: float = Field(..., ge=0, description="Average cost price.")
    marketPrice: float = Field(..., ge=0, description="Current market price.")
    marketValue: float = Field(..., description="Current market value (can be negative for short positions).")
...
class PnlDetail(BaseModel):
    ...
    realizedToday: Optional[float] = Field(None, description="Realized P&L for the current session/timeframe.")
    unrealizedToday: Optional[float] = Field(None, description="Unrealized P&L change for the current session/timeframe.")
...
class AccountPnl(BaseModel):
    ...
    timeframe: str = Field(..., description="Requested timeframe, e.g. 'INTRADAY', '1D', 'MTD', 'YTD'.")
    ...
    bySymbol: Dict[str, PnlDetail] = Field(default_factory=dict, description="Map of symbol → PnlDetail.")
...
class OrderSpec(BaseModel):
    ...
    quantity: float = Field(..., gt=0, description="Absolute quantity (units, shares, contracts). Must be positive.")
    orderType: str = Field(..., description="Order type.")
    limitPrice: Optional[float] = Field(None, ge=0, description="Limit price, required for LMT and STP_LMT.")
    stopPrice: Optional[float] = Field(None, ge=0, description="Stop trigger price, required for STP and STP_LMT.")
...
class OrderPreview(BaseModel):
    ...
    estimatedCommission: Optional[float] = Field(None, ge=0, description="Estimated commission and fees.")
...
class OrderStatus(BaseModel):
    ...
    warnings: List[str] = Field(default_factory=list, description="Any broker or system warnings tied to this order.")
    ...
    def validate_status(cls, v: str) -> str:
        ...
        allowed = {
            "PENDING_SUBMIT", "PENDING_CANCEL", "SUBMITTED", "PARTIALLY_FILLED",
...
class CancelResult(BaseModel):
    ...
    def validate_cancel_status(cls, v: str) -> str:
        ...
        if v not in {"CANCELLED", "ALREADY_FILLED", "NOT_FOUND", "REJECTED"}:
            raise ValueError(f"status must be one of {{CANCELLED, ALREADY_FILLED, NOT_FOUND, REJECTED}}, got {v}")
        return v
