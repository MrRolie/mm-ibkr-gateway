...
def healthcheck() -> bool:
    ...
    try:
        # Attempt connection
        print(f"Connecting to IBKR Gateway at {config.ibkr_gateway_host}:{config.ibkr_gateway_port}...")
        ...
        # Verify trading mode
        print(f"[OK] Trading mode: {config.trading_mode.upper()}")
        print(f"[OK] Orders enabled: {config.orders_enabled}")
        
        return True
        
    except ConnectionRefusedError:
        print(f"ERROR: Connection refused. Is IBKR Gateway/TWS running on port {config.ibkr_gateway_port}?")
        return False
    except TimeoutError:
        print(f"ERROR: Connection timeout. IBKR Gateway/TWS not responding on port {config.ibkr_gateway_port}.")
        return False
